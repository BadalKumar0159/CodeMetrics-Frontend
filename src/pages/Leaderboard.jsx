...
import { Trophy, Medal, Code, Activity, ChevronUp, ExternalLink, Loader, RefreshCw, Filter, User, Award, ChevronsUp, Github, Plus, } from "lucide-react";
...
const Leaderboard = () => {
   ...
   const sortUserData = useCallback((userData) => {
   ...
   const buildLeaderboardDataForUser = useCallback(async (user, userInfo, delay) => {
      await new Promise(res => setTimeout(res, delay));

      try {
         const submissionResponse = await fetch(`https://codeforces.com/api/user.status?handle=${user}&from=1&count=100`);

         if (!submissionResponse.ok)
            throw new Error(`Submissions API failed: ${submissionResponse.status}`);

         const submissionData = await submissionResponse.json();
         if (user == 'badalkumar0159')
            console.log(submissionData);

         const rangeCutoff = Date.now() - timeRange * 24 * 60 * 60 * 1000;
         const recentSubmissions = submissionData.result?.filter((sub) => sub.creationTimeSeconds * 1000 >= rangeCutoff) || [];
         ...
         return {
            ...
            activeDays: submissionDays.size,
   ...
   const fetchUserInfoBatch = useCallback(async () => {
      ...
      const batches = [];     // Split into batches to avoid API limitations
      ...
      for (const batch of batches) {
         // Extract valid usernames and join them into a ';' separated string for the Codeforces API
         const handles = batch.map((user) => user.codeforcesUsername).filter(Boolean).join(";");
         if (!handles) continue;
         ...
         try {
            ...
            if (!response.ok) {
               ...
               continue;       // Continue with other batches rather than failing completely
            }
            ...
            errorCount++;
            console.error("Error fetching batch:", error);      // Continue with other batches
         }

         if (batches.length > 1) {                              // Add delay between batches to avoid rate limiting
   ...
   const fetchUserData = useCallback(async () => {
      ...
      try {
         const userInfo = await fetchUserInfoBatch();       // Fetch user info in batch
         const promises = [];

         usersList.forEach((user, index) => {
            promises.push(buildLeaderboardDataForUser(user.codeforcesUsername, userInfo[user.codeforcesUsername] || {}, index * 300));
         });
   ...
   }, [usersList, fetchUserInfoBatch, buildLeaderboardDataForUser]);

   const handleRefresh = () => {
      fetchUserData();
   };

   // Derive the sorted leaderboard from users instead of storing sorted data in state.
   const sortedUsers = useMemo(() => {
      return sortUserData(users);
   }, [users, sortUserData]);

   useEffect(() => {            // Fetch user data when usersList or timeRange changes
      if (usersList.length > 0) {
         fetchUserData();
      } else {
         setLoading(false);
   ...
   useEffect(() => {
      let isMounted = true;    // Prevents state updates after the component unmounts.
      ...
      const fetchUsers = async () => {
         ...
         if (response.success)
            setUsersList(response.arr.map((username) => ({ codeforcesUsername: username })));
         else
      ...
      return () => { isMounted = false; };  // called before the effect runs again & when the component unmounts
   }, [userId]);
         ...
         <div className="max-w-6xl mx-auto">
            ...
            <div className="bg-gray-800/50 backdrop-blur-md shadow-xl rounded-xl p-4 mb-8 flex flex-wrap items-center justify-between gap-4 border border-gray-700/80 hover:border-gray-700">
               <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <select value={timeRange} onChange={(e) => setTimeRange(Number(e.target.value))} className="bg-gray-700 text-white border 
                     border-gray-600 rounded-lg px-2 py-1.5 text-sm focus:ring-2 focus:ring-cyan-600 focus:border-transparent" >
            ...
            {loading ? (
               ...
               <div className="text-center p-12 bg-red-900/20 backdrop-blur-md rounded-xl shadow-xl border border-red-900/50">
                  ...
                  <button onClick={handleRefresh} className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 
                  ...
                  {users.length === 0 ? (
                     <div className="text-center p-16 bg-gray-800/50 backdrop-blur-md rounded-xl shadow-xl border border-blue-900/50">
                        ...
                        <p className="text-gray-400 mb-6 text-sm">
                           Add some users to track.
                        </p>
                        <button onClick={() => navigate(-1)} className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 
                        text-white px-5 py-2.5 rounded-lg text-sm flex items-center gap-2 mx-auto transition-all shadow-lg hover:shadow-xl" >
                           <Plus size={18} />
                           <span>Add User</span>
                        </button>
                     </div>
                           ...
                           <table className=" w-full">
                                 ...
                                 <tr>
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-cyan-400 uppercase tracking-wider">
                                       <div className="flex items-center gap-1">
                                          ...
                                          {sortBy === "activeDays" && <ChevronUp size={14} />}
                                       </div>
                                    </th>
                                    <th className="px-6 py-4 text-xs font-semibold text-cyan-400 uppercase tracking-wider">
                                       <div className="flex items-center gap-1">
                              ...
                              <tbody className="divide-y divide-gray-700/60">
                                 {sortedUsers.map((user, index) => (
                                    <tr key={user.id || `user-${index}`} className={`hover:bg-blue-900/30 transition-all ${index < 3 ? "font-medium" : ""}`} >
                                       <td className="px-6 py-4 whitespace-nowrap">
                                          <div className={`w-10 h-10 flex items-center justify-center rounded-full ${getMedalColor(index)} shadow-lg 
                                          ${index < 3 ? "ring-2 ring-blue-400/50 ring-offset-2 ring-offset-gray-800" : ""}`} >
                                             {getRankIcon(index)}
                                          </div>
                                       </td>
                                       <td className="px-6 py-4 whitespace-nowrap">
                                          <div onClick={() => user.username && navigate(`/user-detail/${user.username}`)} className={`flex items-center gap-2 
                                          ${user.username ? "cursor-pointer hover:text-cyan-300 transition-colors" : ""}`} >
                                       ...
                                       <td className="px-6 py-4 whitespace-nowrap">
                                          <div className="flex items-center gap-2">
                                             ...
                                             <span className={`tabular-nums font-medium ${getCFRankColor(user.rank)}`} >
                                       ...
                                       <td className="px-6 py-4 whitespace-nowrap">
                                          {user.profileUrl && user.profileUrl !== "#" ? (
                                             <a href={user.profileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center text-blue-400 
                                             hover:text-cyan-300 transition-colors bg-gray-700/50 hover:bg-gray-700 p-2 rounded-full" >
